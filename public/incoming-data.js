...
function renderRodCharts(rows) {
  ...
  // 2) 일별 평균 와블 추이 (업체별 + 기준선)
  makeLine('rodWobbleTrend', $('inq-rod-wobble-trend').getContext('2d'), dates, [
    ...sups.map((s, i) => ({
      label: s,
...
window.generateRodReport = function () {
  ...
  // 1) 일별 평균 테이퍼 높이 추이 (STATE.rods 전체 누적, 업체별)
  const _allDates = uniq(STATE.rods.map(r => r.measure_date).filter(Boolean)).sort();
  const _allSups = [...new Set(STATE.rods.map(r => r.supplier).filter(Boolean))].sort();
  const _rPalette = ['#002BD2','#54DBC2','#00b87a','#e6a800','#FF6C39','#7c5fe6','#3C7DFF','#94a3b8'];
  const imgTrend = _makeReportChartSync({
    type: 'line',
    data: { labels: _allDates, datasets: [
      ..._allSups.map((s, i) => ({
        label: s,
        data: _allDates.map(d => { const vs = STATE.rods.filter(r => r.measure_date === d && r.supplier === s).map(rodH).filter(v => v !== null); return vs.length ? +avg(vs).toFixed(2) : null; }),
        borderColor: _rPalette[i % 8], backgroundColor: _rPalette[i % 8] + '20',
        tension: 0.3, pointRadius: 2, borderWidth: 2, spanGaps: true, fill: false
      })),
      { label: '상한 (7.0)', data: _allDates.map(() => 7.0), borderColor: '#FF6C39', borderWidth: 1, borderDash: [5, 4], pointRadius: 0, fill: false },
      { label: '하한 (5.0)', data: _allDates.map(() => 5.0), borderColor: '#FF6C39', borderWidth: 1, borderDash: [5, 4], pointRadius: 0, fill: false },
    ]},
    options: {
      scales: {
        x: { grid: { display: false }, ticks: { maxRotation: 45, minRotation: 45, font: { size: 9 } } },
        y: { suggestedMin: 4.5, suggestedMax: 7.5, title: { display: true, text: '높이 (mm)', font: { size: 10 } }, grid: { color: '#E2E2EA' } }
      },
      plugins: { legend: { display: true, position: 'top', align: 'end', labels: { boxWidth: 12, font: { size: 10 } } }, datalabels: { display: false } }
    }
  }, 900, 280);

  // 2) 일별 평균 와블 추이 (STATE.rods 전체 누적, 업체별)
  const imgWobbleTrend = _makeReportChartSync({
    type: 'line',
    data: { labels: _allDates, datasets: [
      ..._allSups.map((s, i) => ({
        label: s,
        data: _allDates.map(d => { const vs = STATE.rods.filter(r => r.measure_date === d && r.supplier === s).map(r => r.wobble).filter(v => v != null); return vs.length ? +avg(vs).toFixed(2) : null; }),
        borderColor: _rPalette[i % 8], backgroundColor: _rPalette[i % 8] + '20',
        tension: 0.3, pointRadius: 2, borderWidth: 2, spanGaps: true, fill: false
      })),
      { label: '와블 한계 (1.0)', data: _allDates.map(() => 1.0), borderColor: '#FF6C39', borderWidth: 1, borderDash: [5, 4], pointRadius: 0, fill: false },
    ]},
    options: {
      scales: {
        x: { grid: { display: false }, ticks: { maxRotation: 45, minRotation: 45, font: { size: 9 } } },
        y: { beginAtZero: true, suggestedMax: 1.5, title: { display: true, text: '와블 (mm)', font: { size: 10 } }, grid: { color: '#E2E2EA' } }
      },
      plugins: { legend: { display: true, position: 'top', align: 'end', labels: { boxWidth: 12, font: { size: 10 } } }, datalabels: { display: false } }
    }
  }, 900, 280);
...
${recHtml}
<p style="font-weight: bold; font-size: 10pt; margin: 25px 0 8px 0;">3. 검사 결과 그래프</p>
${imgTrend ? `<p style="margin: 3px 0 6px 15px; font-size: 10pt;">1) 일별 평균 테이퍼 높이 추이 (누적)</p>
<img src="${imgTrend}" style="width:100%;margin:4px 0 20px 0;display:block;border:1px solid #ccc;">` : ''}
${imgWobbleTrend ? `<p style="margin: 3px 0 6px 15px; font-size: 10pt;">2) 일별 평균 와블 추이 (누적)</p>
<img src="${imgWobbleTrend}" style="width:100%;margin:4px 0 20px 0;display:block;border:1px solid #ccc;">` : ''}
${imgSupplierH ? `<p style="margin: 3px 0 6px 15px; font-size: 10pt;">3) 공급업체별 평균 테이퍼 (기준 5.0~7.0) — ${period}</p>
<img src="${imgSupplierH}" style="width:100%;margin:4px 0 20px 0;display:block;border:1px solid #ccc;">` : ''}
${imgSupplierW ? `<p style="margin: 3px 0 6px 15px; font-size: 10pt;">4) 공급업체별 평균 와블 (기준 ≤1.0) — ${period}</p>
<img src="${imgSupplierW}" style="width:100%;margin:4px 0 20px 0;display:block;border:1px solid #ccc;">` : ''}
</body>
</html>`;
...
function _dlDetailRow(no, d) {
  ...
  return '<tr id="dldr-' + id + '">'
    ...
    + '<td style="' + td + ';min-width:75px">' + _inp(d.inbound, 'text-align:right', 'number') + '</td>'
    + '<td style="' + td + ';min-width:75px">' + _inp(d.return_qty, 'text-align:right', 'number') + '</td>'
    + '<td style="' + td + ';min-width:75px">' + _inp(d.pass_qty, 'text-align:right', 'number') + '</td>'
    + '<td style="' + td + ';min-width:90px">' + _inpList(d.inspector || '김철우', 'dl-inspector-list') + '</td>'
    + '<td style="' + td + ';min-width:120px">' + _inpList(d.defect_type || '', 'dl-deftype-list', 'min-width:120px') + '</td>'
...
async function _dlPopulateAutoComplete() {
  try {
    var url = SB_URL + '/rest/v1/daily_log_details?select=company,code,name&order=log_date.desc';
    var data = await _fetchAllPaginated(url);
    var seen = { company: {}, code: {}, name: {} };
    data.forEach(function(d) {
      if (d.company) seen.company[d.company] = 1;
      if (d.code) seen.code[d.code] = 1;
      if (d.name) seen.name[d.name] = 1;
    });
    var fill = function(id, keys) {
      var el = document.getElementById(id);
      if (el) el.innerHTML = keys.sort().map(function(v) { return '<option value="' + escHtml(v) + '">'; }).join('');
    };
    fill('dl-company-list', Object.keys(seen.company));
    fill('dl-code-list', Object.keys(seen.code));
    fill('dl-name-list', Object.keys(seen.name));
  } catch(e) {}
}
...
function _dlRender(date, sum, details, notes) {
  ...
  let section2 = '<div style="background:var(--sidiz-card);border:1px solid var(--border);border-radius:12px;padding:18px 20px;margin-bottom:16px">'
    ...
    + '</div>'
    + '<datalist id="dl-company-list"></datalist>'
    + '<datalist id="dl-code-list"></datalist>'
...
window.dlSave = async function () {
  ...
  document.querySelectorAll('#dl-detail-body tr:not(#dl-detail-empty)').forEach(function(tr, i) {
    ...
    detailRows.push({ log_date: date, no: i + 1, company: v(0), code: v(1), name: v(2), judge: v(3), inbound: nv(4), return_qty: nv(5), pass_qty: nv(6), inspector: v(7), defect_type: v(8), defect_info: v(9), action: v(10) });
  });
...
function _dlRenderCharts(summaries, details, year, yearSummaries) {
  ...
  var mDefects = mLabels.map(function(lbl, i) {
    var key = year + '-' + String(i + 1).padStart(2, '0');
    var m = monthlyMap[key];
    return m ? m.defect : 0;
  });
  ...
  var defTypeMap = {};
  fDetails.forEach(function(d) { var key = d.defect_type || d.defect_info; if (key && d.judge !== '합격') defTypeMap[key] = (defTypeMap[key] || 0) + 1; });
        ...
        for (var di = 0; di < dates.length; di++) {
          ...
          failBulk.forEach(function(row, idx) {
            ...
            if (d.company || d.name || d.code) {
              ...
              detailBulk.push({ log_date: date, no: idx+1, company: d.company||'', code: d.code||'', name: d.name||'', judge: d.judge||'', inbound: Number(d.inbound)||0, return_qty: Number(d.return_qty)||0, pass_qty: Number(d.pass_qty)||0, inspector: d.inspector||'김철우', defect_type: d.defect_type||'', defect_info: d.defect_info||'', action: aVal });
            }
          });
...
window.dlGenerateReport = async function () {
  ...
  const results = await Promise.all([
    ...
    fetch(SB_URL + '/rest/v1/daily_log_summary?log_date=gte.' + year + '-01-01&log_date=lte.' + year + '-12-31&order=log_date.asc', { headers: SB_HEADERS }).then(function(r) { return r.json(); }),
  ]);
  const summaries = results[0], allNotes = results[1], allDetails = results[2], yearSummaries = results[3];
  ...
  var imgDaily = _makeReportChartSync({
    type:'bar',
    data:{labels:dLabels,datasets:[
      {type:'bar',label:'불합격수량',data:dDefects,backgroundColor:'rgba(255,108,57,0.85)',barPercentage:0.6,yAxisID:'y',datalabels:{display:function(ctx){return ctx.raw>0;},anchor:'end',align:'top',formatter:function(v){return v;},font:{size:9,weight:'bold'},color:'#FF6C39'}},
      {type:'line',label:'불량율(%)',data:dRates,borderColor:'#FF3A4A',borderDash:[5,3],backgroundColor:'rgba(0,0,0,0)',tension:0.3,pointRadius:3,yAxisID:'y1',datalabels:{anchor:'top',align:'top',formatter:function(v){return v.toFixed(2)+'%';},font:{size:10,weight:'bold'},color:'#FF3A4A'}}
    ]},
    options:{responsive:false,plugins:{legend:{labels:{font:{size:10},boxWidth:12}}},scales:{x:{ticks:{font:{size:9}}},y:{min:0,suggestedMax:Math.ceil(Math.max.apply(null,dDefects.concat([1]))*1.35),ticks:{font:{size:9}}},y1:{min:0,suggestedMax:Math.max.apply(null,dRates.concat([0.01]))*1.5,position:'right',grid:{drawOnChartArea:false},ticks:{font:{size:9},callback:function(v){return v+'%';}}}}}
  }, 860, 260);
  ...
  allDetails.forEach(function(d) {
    if (d.judge !== '합격') {
      var dk = d.log_date.substring(5);
      if (!defDayMap[dk]) defDayMap[dk] = [];
      defDayMap[dk].push(d);
    }
  });
  var defDayKeys = Object.keys(defDayMap).sort();
  var defDayRows = '';
  defDayKeys.forEach(function(dk) {
    defDayMap[dk].forEach(function(d, idx) {
      defDayRows += '<tr>'
        + (idx === 0 ? '<td style="' + tdC + ';white-space:nowrap;vertical-align:top" rowspan="' + defDayMap[dk].length + '">' + dk + '</td>' : '')
